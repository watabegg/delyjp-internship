...
import { z } from "zod";
import type { PayloadMap } from "../types/express";
...
${
	attrs.ingredients && Array.isArray(attrs.ingredients)
		? attrs.ingredients
				.map((ing: { name?: string; amount?: string } | string) =>
					typeof ing === "string"
						? `• ${ing}`
...
${
	attrs.instructions && Array.isArray(attrs.instructions)
		? attrs.instructions
				.map(
					(step: { description?: string } | string, i: number) =>
						`${i + 1}. ${
...
export function createVideoControlTool() {
	return new DynamicStructuredTool({
		...
		func: async ({ instruction, time }) => {
			...
			const payload: PayloadMap["videoControll"] = { instruction };
			if (time !== undefined) {
				payload.time = time;
			}
