...
const clients = globalThis.videoClients ?? (globalThis.videoClients = new Map<string, ReadableStreamDefaultController>());

// クライアントを登録
export function registerClient(recipeId: string, controller: ReadableStreamDefaultController) {
  ...
  console.log(`[SSE LIB] 登録されたクライアントIDs:`, Array.from(clients.keys()));
}
...
export function sendVideoControlEvent(recipeId: string, event: {
  ...
  console.log(`[SSE] 登録されているクライアントIDs:`, Array.from(clients.keys()));
