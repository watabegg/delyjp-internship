...
export function VideoController({
	...
	useEffect(() => {
		...
		const eventSource = new EventSource(
			`/api/video-events?recipeId=${recipeId}`,
		);
					...
					switch (data.instruction) {
						case "PLAY":
							video.play().catch((error) => {
								if (error.name === "NotAllowedError") {
									console.warn(
										"⚠️ 自動再生が制限されています。ユーザーが動画をクリックして最初に再生してください。",
									);
						...
						case "REWIND": {
							const rewindTime = Math.max(
								0,
								video.currentTime - (data.time || 10),
							);
							console.log("[VIDEO] 巻き戻し実行前:", {
								currentTime: video.currentTime,
							});
							video.currentTime = rewindTime;
							console.log("[VIDEO] 巻き戻し実行後:", {
								currentTime: video.currentTime,
								targetTime: rewindTime,
							});
							break;
						}
						case "FORWARD": {
							...
							console.log("[VIDEO] 早送り実行前:", {
								currentTime: video.currentTime,
							});
							video.currentTime = forwardTime;
							console.log("[VIDEO] 早送り実行後:", {
								currentTime: video.currentTime,
								targetTime: forwardTime,
							});
							break;
						}
						default:
	...
	}, [recipeId, showAutoplayWarning, videoSrc]);
