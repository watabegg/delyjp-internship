  ...
  useEffect(() => {
    const interval = setInterval(async () => {
      try {
        ...
        console.log("[TTS] ポーリング結果:", data, "現在のテキスト:", currentText);
        
        if (data.text && data.text !== currentText) {
          console.log("[TTS] 新しいテキスト受信:", data.text, "前回:", currentText);
          ...
          if (isSupported) {
            ...
            setTimeout(() => {
              // テキストを短く区切って確実性を向上
              const text = data.text.length > 100 ? data.text.substring(0, 100) + "..." : data.text;
              ...
              if (japaneseVoice) {
                ...
                console.log("[TTS] 日本語音声設定:", japaneseVoice.name, japaneseVoice.lang);
              } else {
                console.log("[TTS] 日本語音声が見つからない、利用可能な音声:", voices.map(v => `${v.name} (${v.lang})`));
              }
