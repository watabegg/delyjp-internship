...
export default function VoiceInterface({ onStatusChange, onTranscript, onResponse }: VoiceInterfaceProps = {}) {
  const { status, isRecording, transcript, response, connect, startRecording, stopRecording } = useVoiceSession({
    onStatusChange,
    onTranscript,
    onResponse,
  });
  ...
  return (
    <div className="voice-interface p-6 max-w-md mx-auto bg-white rounded-lg shadow-lg">
      ...
      {status === "disconnected" && (
        <button onClick={connect} className="w-full py-2 px-4 bg-blue-500 text-white rounded hover:bg-blue-600 mb-4">
          接続する
        </button>
      )}
      ...
      {response && (
        <div className="response mt-4 p-3 bg-blue-50 rounded">
          <h4 className="font-medium text-sm text-blue-700 mb-1">アシスタント:</h4>
